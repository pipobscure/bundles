...
// A `node:vfs` file provider for signed archives — `.napp` files — layered on
...
export function open(path, options) {
    ...
    try {
        const res = verifySync(resolved, { archive, deep: opts.deep, extraRoots: opts.extraRoots });
        const acceptable = res.state === 'valid' || (opts.allowUntrusted && res.state === 'valid-untrusted');
        if (!acceptable) {
            throw refusal(resolved, res);
        }
        return new NappProvider(archive, { hashAlg: res.hashAlg, digests: res.digests });
    } catch (err) {
        archive.closeSync();
        throw err;
    }
...
//
// options.extensions     - file suffixes claimed outright (default: ['.napp'])
...
export class NappProvider extends VFS.ZipProvider {
    #archive;
    #hashAlg;
    #digests;
    #verified;
    ...
    #check(name) {
        ...
        if (actual !== recorded) {
            throw Object.assign(new Error(
                `napp: content of '${name}' does not match its signed digest ` +
                `(expected ${recorded}, got ${actual})`), { code: 'ERR_NAPP_INTEGRITY', member: name });
        }
...
function refusal(path, res) {
    ...
    return Object.assign(new Error(`napp: refusing to mount '${path}': ${res.state} — ${detail}`),
        { code: 'ERR_NAPP_UNTRUSTED', state: res.state });
}
