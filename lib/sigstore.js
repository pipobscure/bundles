...
import { identityToken, FULCIO_AUDIENCE } from './oidc.js';
...
// hashes it. `lib/archive.js` drives exactly that sequence.
//
...
// those establish, which is why the sigstore path below replaces the plain
// X.509 anchoring rather than adding to it.
//
...
// Whether the sigstore libraries are installed and loadable.
...
//
// options.token       - an OIDC token to use instead of signing in
// options.flow        - 'auto' | 'ci' | 'browser' | 'device'
// options.issuer      - OIDC issuer (default: sigstore's Dex)
// options.connector   - Dex connector to jump to (default: 'github')
// options.fulcioURL   - certificate authority (default: Fulcio)
// options.rekorURL    - transparency log; '' to skip
// options.tsaURL      - RFC 3161 timestamp authority; '' to skip
// options.signAlg     - digest the signature uses (default: 'sha256')
// options.log         - progress reporter
export async function signer(options = {}) {
...
async function certify(token, keypair, fulcioURL) {
    ...
    const proof = CRYPTO.sign(null, Buffer.from(subject), keypair.privateKey);
...
async function witness({ artifact, signature, certificate, rekorURL, tsaURL, log }) {
    const { toMessageSignatureBundle, bundleToJSON } = load('@sigstore/bundle');
    const { RekorWitness, TSAWitness } = load('@sigstore/sign');

    const bundle = toMessageSignatureBundle(
        { data: artifact },
        { signature, key: { $case: 'x509Certificate', certificate } },
    );
    ...
    const tlogEntries = [];
    const rfc3161Timestamps = [];
    for (const [label, w] of witnesses) {
        try {
...
// `provider.js` decides whether to serve an archive before any of the program
// in it runs, and cannot await.
//
// Returns { identity, issuer, signedAt } on success. Throws on a bundle that
// does not verify. `trustedRoot` is the sigstore trust root, which must be
// supplied — this deliberately does not reach for the network.
export function verifyBundle(encoded, artifact, { trustedRoot, identity, issuer } = {}) {
    const { bundleFromJSON } = load('@sigstore/bundle');
    const { Verifier, toTrustMaterial, toSignedEntity } = load('@sigstore/verify');
    ...
    return {
        ...
        certificate: bundle.verificationMaterial?.content?.certificate,
    };
}

// The leaf certificate a sigstore bundle carries, as PEM — so a verifier can
// confirm the inspectable `AUTHORITY.PEM` names the same certificate the
// bundle was actually verified against.
export function bundleCertificate(encoded) {
    const { bundleFromJSON } = load('@sigstore/bundle');
    const bundle = bundleFromJSON(JSON.parse(Buffer.from(encoded, 'base64').toString('utf-8')));
...
// Load the sigstore trust root synchronously, from an explicit path, the
// `BUNDLE_SIGSTORE_ROOT` environment variable, or the cache `@sigstore/tuf`
// maintains. Returns null when there is none — verification then reports that
// as a reason rather than failing, because "I could not check" and "this is
// forged" are different answers.
export function trustedRootSync(source) {
    const path = source ?? process.env.BUNDLE_SIGSTORE_ROOT ?? cachedRootPath();
    if (!path) return null;
    try {
        const { TrustedRoot } = load('@sigstore/protobuf-specs');
        return TrustedRoot.fromJSON(JSON.parse(FS.readFileSync(path, 'utf-8')));
    } catch {
        return null;
    }
}
...
export async function refreshTrustedRoot({ mirror = DEFAULT_TUF_MIRROR, force = true } = {}) {
    const { getTrustedRoot } = load('@sigstore/tuf');
...
export function cachedRootPath(mirror = DEFAULT_TUF_MIRROR) {
...
function claims(token) {
    try {
        return JSON.parse(Buffer.from(String(token).split('.')[1], 'base64url').toString('utf-8'));
    } catch {
        return {};
    }
}
...
function describe(token) {
    ...
    return {
        ...
        workflow: c.workflow_ref ?? c.job_workflow_ref,
    };
}
