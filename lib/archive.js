...
// `out`. With no prefix the result is a plain archive — a `.napp` meant to be
// run through `--vfs-mount`; with one it is a self-running container (a shebang
// launcher or a SEA binary) that carries the same archive in its tail. When
// `key` and `chain` are given, the whole file is signed. The hash runs over the
