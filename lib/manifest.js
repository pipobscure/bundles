...
export function verifySync(source, { extraRoots, now = Date.now(), deep = true, archive } = {}) {
    const io = Buffer.isBuffer(source) ? bufferSource(source) : pathSource(source);
    const zf = archive ?? io.open();
    try {
        return inspect(zf, io, { extraRoots, now, deep });
    } finally {
...
function inspect(zf, io, { extraRoots, now, deep }) {
