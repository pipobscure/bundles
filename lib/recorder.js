...
//
//   node -r @pipobscure/napp/record --vfs-load --vfs-mount ./lib
//
...
//
//   NAPP_MANIFEST=app.manifest node -r @pipobscure/napp/record \
...
//
// options.manifest - where to write the file list (default: the NAPP_MANIFEST
//                    environment variable)
// options.truncate - start a fresh list (default: true on the main thread)
// options.name     - identifier reported in diagnostics (default: 'napp-manifest')
export function register(options = {}) {
    const configured = options.manifest ?? process.env.NAPP_MANIFEST;
    ...
    VFS.registerProvider({
        name: options.name ?? 'napp-manifest',
        // Directory mounts only: an archive mount has its own provider (and,
        // for a `.napp`, one that verifies what it serves), and quietly
